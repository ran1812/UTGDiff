...
parser.add_argument("--ckpt_path", default='./new_save/35.MOI-GEN/best(1109999).th', type=str, required=False)
...
def collate_fn(batch_input):
    ...
    target_graph_ids = pad_sequence([torch.tensor([tokenizer.encode('<gstart>')[1]] + [tokenizer.mask_token_id] * 127) for d in batch_input], batch_first=True)
...
with torch.no_grad():
    for i, batch in enumerate(tqdm(test_loader)):
        ...
        with open(f'./generation_results/{task_name}_{args.topk}_MBR_(35_110).txt', 'a+') as f_mbr:
