...
def parse_args():
    ...
    parser.add_argument("--save_path", default='./new_save/42.retro_new/', type=str, required=False)
...
if __name__ == '__main__':
    ...
    #model.load_state_dict(torch.load('./new_save/41.fp_pretrain/best(399999).th',map_location=device)['model'])
    '''
    ckpt = torch.load('./new_save/41.fp_pretrain/best(329999).th',map_location=device)['model']
    new_checkpoint = {} 
    ...
    train_data = Loader(tokenizer=tokenizer).my_load(splits=['train'])[0]
    ...
    for epoch in range(args.epochs):
        if epoch ==90 or epoch ==150 or epoch == 180:
        ...
        for batch in tqdm(train_loader):
            ...
            if dist.get_rank() == 0:
                if i % args.logging_steps == args.logging_steps - 1:
                    ...
                    logger.info(f'accumulation step {i} is {args.accumulation_steps}')
                    logger.info('\n')
                    train_loss = .0
                    hybrid_loss_token = .0
                    base_loss_token = .0
            ...
            if i % args.eval_steps == args.eval_steps - 1:
                ...
                if dist.get_rank() == 0:
                    torch.save({
                        'model': model.state_dict(),
                    }, f'./new_save/42.retro_new/best({i}).th')
                model.train()
