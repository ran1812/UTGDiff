...
from dataloader import QQPLoader, QTLoader, CHEBILoader, charge_num2str, MOIfpLoader
import diffusion_condition_new as diffusion
import functools
...
from dataloader import MOIGENLoader,MOIretroLoader
import random
import numpy as np
...
parser.add_argument("--task_name", default='MOIretro', type=str, required=False)
parser.add_argument("--ckpt_path", default='./new_save/40.retro_pretrain/best(859999).th', type=str, required=False)
...
parser.add_argument("--seed", default=42, type=int, required=False, help='Max seq length in generation')
...
def denoise_fn(input_ids, corrupted_input_ids, timestep, attention_mask, target_mask, edge_input, target_start, position_ids,
        source_start, source_edge):
    
    new_input_ids = torch.where(target_mask.bool(), corrupted_input_ids, input_ids)
    output = model(input_ids=new_input_ids, edge_input = edge_input, attention_mask=attention_mask, 
        target_start = target_start, position_ids=position_ids, source_start = source_start, source_edge = source_edge)
...
def collate_fn(batch_input):
    ...
    target_graph_ids = pad_sequence([torch.tensor([tokenizer.encode('<gstart>')[1]] + [tokenizer.mask_token_id] * 95) for d in batch_input], batch_first=True)
    ...
    edge_init = torch.ones_like(target_edge_input) * (cfg.num_edges_type)
    if 'source_node' in batch_input[0]:
        return {
            ...
            'source_edge_input' : source_edge_input.repeat(1, MBR_size, 1).view(-1, source_edge_input.size(-1),source_edge_input.size(-1)),
            'source_start': text_ids.size(1) + 1,
            'target_edge_input' : target_edge_input.repeat(1, MBR_size, 1).view(-1, target_edge_input.size(-1),target_edge_input.size(-1)),
            'edge_init': edge_init.repeat(1, MBR_size, 1).view(-1, target_edge_input.size(-1),target_edge_input.size(-1)),
...
with torch.no_grad():
    for i, batch in enumerate(tqdm(test_loader)):
        for k, v in batch.items():
            if k != 'target_start' and k != 'source_start':
                batch[k] = v.to(device)

        position_ids_text = create_position_ids_from_input_ids(batch['input_ids'][:,:batch['source_start']-1],cfg.pad_token_id)
        position_ids_graph_source = create_position_ids_from_input_ids(batch['input_ids'][:,batch['source_start']-1:batch['target_start']-1],cfg.pad_token_id)
        position_ids_graph_target = create_position_ids_from_input_ids(batch['input_ids'][:,batch['target_start']-1:],cfg.pad_token_id)
        position_ids = torch.cat((position_ids_text,position_ids_graph_source,position_ids_graph_target),dim = -1)
        ...
        outputs = diffusion.discrete_diffusion_predict_fn(
            ...
            denoise_fn=functools.partial(
                        ...
                        source_start = batch['source_start'] if 'source_start' in batch else None,
                        source_edge = batch['source_edge_input'] if 'source_edge_input' in batch else None,
                    ),
            edge = batch['edge_init'],
        ...
        edge = outputs['edge']#.view(batch_size, MBR_size, -1)
        generated_smiles = graph2smile(state,edge,tokenizer,target_start=batch['target_start'])
        with open(f'./generation_results/{task_name}_{args.topk}_MBR_(40_85_13).txt', 'a+') as f_mbr:
