...
def parse_args():
    ...
    parser.add_argument("--save_path", default='./new_save/39.retro_new/', type=str, required=False)
...
if __name__ == '__main__':
    ...
    #model.load_state_dict(torch.load('./new_save/39.retro_new/best(399999).th',map_location=device)['model'])
    '''
    ckpt = torch.load('./new_save/39.retro_new/best(329999).th',map_location=device)['model']
    new_checkpoint = {} 
        ...
        for batch in tqdm(train_loader):
            ...
            if i % args.eval_steps == args.eval_steps - 1:
                ...
                if dist.get_rank() == 0:
                    torch.save({
                        'model': model.state_dict(),
                    }, f'./new_save/39.retro_new/best({i}).th')
                model.train()
