...
from dataloader import MOIGENLoader,MOIretroLoader, MOIfpLoader, MOIrpLoader
...
def parse_args():
    ...
    parser.add_argument("--save_path", default='./new_save/38.retro_semipos/', type=str, required=False)
...
if __name__ == '__main__':
    ...
    tokenizer.add_tokens(['<gstart>', '<gend>','<gsep>'])
    ...
    #model.load_state_dict(torch.load('./new_save/38.retro_semipos/best(739999).th',map_location=device)['model'])
    i=-1

    if args.model_name_or_path in ['allenai/scibert_scivocab_uncased'] and args.pretrain:
        unk = tokenizer.convert_tokens_to_ids('[unk]')
        for element in elements:
            for i in range(-4,5):
                token = '<' + element.symbol + charge_num2str[i] + '>'

                special_token_index = tokenizer.encode(token)[1]

                copy_token_index = tokenizer.convert_tokens_to_ids(element.name)
                print(token,special_token_index,copy_token_index)
                if copy_token_index != unk:
                    copy_token_embedding = model.get_input_embeddings().weight[copy_token_index]
                    copy_decoder_weight = model.cls.predictions.decoder.weight[copy_token_index]
                    copy_decoder_bias = model.cls.predictions.decoder.bias[copy_token_index]
                    copy_pred_bias = model.cls.predictions.bias[copy_token_index]
                    model.get_input_embeddings().weight[special_token_index].data.copy_(copy_token_embedding)
                    model.cls.predictions.decoder.weight[special_token_index].data.copy_(copy_decoder_weight)
                    model.cls.predictions.decoder.bias[special_token_index].data.copy_(copy_decoder_bias)
                    model.cls.predictions.bias[special_token_index].data.copy_(copy_pred_bias)
    ...
    #train_data = Loader(tokenizer=tokenizer).my_load(splits=['validation'])
    #dev_data = train_data
    ...
    def denoise_fn(input_ids, corrupted_input_ids, timestep, attention_mask, target_mask, edge_input, target_start,
        ...
        output = model(input_ids=new_input_ids, edge_input = edge_input, attention_mask=attention_mask, 
            target_start = target_start, source_start = source_start, source_edge = source_edge)
        ...
        for batch in tqdm(train_loader):
            ...
            if i % args.eval_steps == args.eval_steps - 1:
                ...
                with torch.no_grad():
                    ...
                    torch.save({
                        'model': model.state_dict(),
                    }, f'./new_save/38.retro_semipos/best({i}).th')
                model.train()
