...
class DiscreteDiffusionMatrixBase(DiscreteDiffusionBase):
    ...
    def sample_and_compute_posterior_q(self,
        ...
        prob_at_time_t = self.get_qt_given_q0(q0=x_0, t=t, word_freq_logits=word_freq_logits, mask_id = mask_id, func_type=func_type)
        ...
        if self.supports_efficient_get():
            ...
            prob_at_time_t_plus_one = self.get_qt_given_q0(q0=x_0, t=t + step_size, word_freq_logits=word_freq_logits, mask_id = mask_id, func_type=func_type,target_start=target_start,corrupt=True)
        ...
        if transition_probs is None:
            if self.supports_efficient_get():
                ...
                if step_size > 1:
                    transition_probs = torch.nn.functional.one_hot(samples, dim)
                    for i in range(step_size):
                    ...
                    transition_probs = self.qt_reverse(qt_plus_1=samples, make_one_hot=True, t=t, mask_id = mask_id, dim = dim)
...
class MaskDiffusion(DiscreteDiffusionMatrixBase):
    def __init__(self,
        ...
        self.word_freq_edge = word_freq_edge.to(device)
    ...
    def noise_fn(self, q0, t, word_freq_logits, mask_id = None,func_type=None,target_start=None,corrupt=False):
        ...
        if word_freq_logits is None:
            ...
            if func_type=='edge':
                tmp_q0 = q0.reshape(q0.shape[0],-1,q0.shape[-1])
                word_freq_logits = self.word_freq_edge.repeat(tmp_q0.size(0), 1).gather(1, tmp_q0.argmax(-1))
                word_freq_logits = word_freq_logits - word_freq_logits.mean(-1, keepdims=True)
                word_freq_logits = word_freq_logits.reshape(q0.shape[0],q0.shape[1],q0.shape[2])
        ...
        '''
        if corrupt:
            corrupt_rate = random.random()*0.15
            corrupt_prob =  non_mask_prob.sum(-1, keepdims=True) * corrupt_rate
            non_mask_prob = non_mask_prob * (1-corrupt_rate)
            if func_type=='edge':
                non_mask_prob += corrupt_prob / non_mask_prob.shape[-1]
            if func_type=='token':
                non_mask_prob[:, :target_start - 1, :mask_id] += corrupt_prob[:,:target_start - 1] / mask_id
                non_mask_prob[:, target_start - 1:, mask_id + 1:] += corrupt_prob[:,target_start - 1:] / (non_mask_prob.shape[-1] - mask_id - 1)
        '''
        prob_at_time_t = torch.cat((
            non_mask_prob[..., :mask_id], mask_prob,
            non_mask_prob[..., mask_id + 1:]
        ), dim=-1)
    ...
    def qt_reverse(self,
        ...
        if make_one_hot:
            ...
            qt_plus_1 = torch.nn.functional.one_hot(qt_plus_1, num_classes=dim)
        
        if mask_id is None:
            mask_id =  self.tokenizer.mask_token_id

        beta = self.schedule(t)
        qtpls1_at_mask = qt_plus_1[Ellipsis, mask_id: mask_id + 1]
        non_mask_prob0 = (1 - beta) * qt_plus_1[Ellipsis, :mask_id] + beta * qtpls1_at_mask
        non_mask_prob1 = (1 - beta) * qt_plus_1[Ellipsis, mask_id + 1:] + beta * qtpls1_at_mask
    ...
    def get_qt_given_q0(self,
        ...
        prob_at_time_t = q0 if t == 0 else self.noise_fn(q0, t, word_freq_logits,mask_id = mask_id,func_type=func_type,target_start=target_start,corrupt=corrupt)
...
def p_forward(
        ...
        transition_probs=None,
        transition_probs_edge = None,
        transition_probs_in_logits=True,
        maximum_likelihood=False,
        epsilon=1e-20,
        step_size=1,
        word_freq_logits=None,
        edge_freq_logits=None,
        target_start = None,
):
    ...
    logits,edge_logits = denoise_fn(corrupted_input_ids=x_t, edge_input = edge, timestep=t)
    ...
    qt_probs_edge, _ = diffusion.sample_and_compute_posterior_q(
        x_0= edge_probs,
        t=t - step_size,
        return_logits=return_logits,
        make_one_hot=maximum_likelihood,
        transition_probs_in_logits=transition_probs_in_logits,
        transition_probs=transition_probs_edge,
        samples=edge,
        epsilon=epsilon,
        step_size=step_size,
        word_freq_logits=edge_freq_logits,
        func_type = 'edge',
    )

    retval_x0 = logits if return_logits else probs
    retval_e0 = edge_logits if return_logits else edge_probs

    retval = qt_probs
    retval_e = qt_probs_edge
    ...
    retval_e = retval_e0 if t == step_size else retval_e

    if return_x0:
        return (retval, retval_x0), (retval_e, retval_e0)
    else:
        return retval, retval_e
...
def compute_kl_reverse_process(x_start,
    ...
    edge = edge.long()
    edge_freq_logits = torch.zeros_like(edge)
    q_t_edge, x_t_plus_1_edge, transition_probs_edge = diffusion.sample_and_compute_posterior_q(
        edge,
        t,
        return_logits=log_space,
        return_transition_probs=True,
    ...
    transition_probs_edge = transition_probs_edge if use_cached_transition else None

    p_t, p_t_edge = p_forward(
        denoise_fn,
        x_t_plus_1,
        t + step_size,
        diffusion,
        edge = x_t_plus_1_edge,
        predict_x0=predict_x0,
        return_x0=predict_x0 and hybrid_lambda > 0.0,
        return_logits=log_space,
        transition_probs=transition_probs,
        transition_probs_edge=transition_probs_edge,
        step_size=step_size,
        word_freq_logits=word_freq_logits,
        edge_freq_logits = edge_freq_logits,
        target_start=target_start,
    )
    
    if predict_x0 and hybrid_lambda > 0.0:
        p_t, p_0 = p_t
        p_t_edge, p_0_edge = p_t_edge
        if log_space:
            cross_entropy = losses.cross_entropy_with_logits(logits=p_0, targets=x_start) +\
                losses.cross_entropy_with_logits(logits=p_0_edge, targets=edge)
        else:
            cross_entropy_token = losses.cross_entropy_with_probs(probs=p_0, targets=x_start)
            cross_entropy_edge = losses.cross_entropy_with_probs(probs=p_0_edge, targets=edge)

        hybrid_loss_token = hybrid_lambda * cross_entropy_token
        hybrid_loss_edge = hybrid_lambda * cross_entropy_edge
    ...
    '''
    hybrid_loss = (hybrid_loss_token).sum() + 1 * (hybrid_loss_edge).sum()

    base_loss_token = (losses.cross_entropy_with_probs(probs=p_t, targets=x_start)).sum() \
        if t == 0 else (losses.kl_divergence_with_probs(q_t, p_t)).sum()

    base_loss_edge = (losses.cross_entropy_with_probs(probs=p_t_edge, targets=edge)).sum() \
        if t == 0 else (losses.kl_divergence_with_probs(q_t_edge, p_t_edge)).sum()

    if torch.isnan(base_loss_edge).any():
        base_loss_edge = torch.tensor(0.0, requires_grad=True)
        print('warning')
    if torch.isnan(base_loss_token).any():
        base_loss_token = torch.tensor(0.0, requires_grad=True)
        print('warning')
    if torch.isnan(hybrid_loss).any():
        hybrid_loss = torch.tensor(0.0, requires_grad=True)
        print('warning')

    base_loss = 1 * base_loss_edge + base_loss_token
    ...
    metrics_dict = {
        ...
        "base_loss": base_loss,
        "hybrid_loss_token": (hybrid_loss_token).sum(),
        "base_loss_token": base_loss_token,
        "hybrid_loss_edge": (hybrid_loss_edge).sum(),
        "base_loss_edge": base_loss_edge,
...
def discrete_diffusion_predict_fn(
  ...
  """
    ...
    class SamplingState:
        ...
        e: torch.Tensor  # current predicted seqeunce
        e0: Any  # only used if predict_x0 is true
        t: int  # current step
    ...
    def sampling_step(step, state):
        ...
        logits, x0_logits = x_results
        logits_edge, x0_logits_edge = e_results

        if x0_logits is not None:
            x0 = x0_logits.argmax(-1)
            x0_edge = x0_logits_edge.argmax(-1)
        else:
            x0 = None
            x0_edge = None
        ...
        logits_edge = logits_edge / temperature
        logits_edge = top_k_top_p_filtering(logits_edge, top_k=topk, top_p=topp)

        sample = torch.distributions.categorical.Categorical(logits=logits).sample()
        sample_edge = torch.distributions.categorical.Categorical(logits=logits_edge).sample()

        true_edge = torch.zeros_like(sample_edge).long()
        for i in range(true_edge.shape[0]):
            true_edge[i] = torch.triu(sample_edge[i], diagonal=0)
            true_edge[i] += torch.triu(sample_edge[i], diagonal=1).T

        if show_process:
            print(tk.batch_decode(x0, clean_up_tokenization_spaces=False))

        return SamplingState(x=sample, x0=x0, e=sample_edge, e0=x0_edge, t=t - step_size)

    x = torch.where(target_mask.bool(), mask_token_id, input_ids)

    if predict_x0:
        init_state = SamplingState(x, x, edge, edge, torch.tensor([num_steps], device=input_ids.device))
    else:
        init_state = SamplingState(x, None, edge, None, torch.tensor([num_steps], device=input_ids.device))
