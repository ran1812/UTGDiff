...
def parse_args():
    ...
    parser.add_argument("--save_path", default='./new_save/50.pretrain_MOIGEN/', type=str, required=False)
...
if __name__ == '__main__':
    ...
    tokenizer = tok_cls.from_pretrained('./pretrain_gimlet')
    ...
    cfg = cfg_cls.from_pretrained('./pretrain_gimlet')
    print(cfg)

    if args.from_scratch:
        model = model_cls(cfg).to(device)
    elif args.load_step <= 0:
        model = model_cls.from_pretrained('./pretrain_gimlet', config=cfg).to(device)
    else:
        model = model_cls(cfg).to(device)
    ...
    #model.load_state_dict(torch.load('./new_save/50.pretrain_MOIGEN/best(759999).th',map_location=device)['model'])
    i=-1
        ...
        for batch in tqdm(train_loader):
            ...
            if i % args.eval_steps == args.eval_steps - 1:
                ...
                with torch.no_grad():
                    ...
                    torch.save({
                        'model': model.state_dict(),
                    }, f'./new_save/50.pretrain_MOIGEN/best({i}).th')
                model.train()
