...
from utils import tokenize_function_gimlet, tokenize_function_mixed, tokenize_function_t5
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained("QizhiPei/biot5-base", model_max_length=512, local_files_only=True)
    ...
    if args.data=='haitengzhao/molecule_property_instruction':
        ...
        final_cache_dir = os.path.join(cache_base_dir, 'final_processed_new')
        ...
        try:
            ...
            tokenized_datasets = tokenized_datasets.remove_columns(['edge'])
            for i in range(10):
                print(tokenized_datasets['text'][i])
                print(tokenized_datasets['type'][i])
                print(tokenized_datasets['input_ids'][i])
                print(tokenized_datasets['edge'][i])
                print(tokenized_datasets['attention_mask'][i])
            exit()
            tokenized_datasets = load_from_disk(final_cache_dir)
            ...
            def pre_func(examples):
                return {'text': examples['selfies'],'type':'graph'}
            ...
            tokenize_function=lambda x: tokenize_function_t5(examples=x, tokenizer=tokenizer, padding=False, max_seq_length=tokenizer.model_max_length)
