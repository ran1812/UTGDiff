...
from dataloader import QQPLoader, QTLoader, CHEBILoader, charge_num2str, PCDesLoader
...
parser.add_argument("--step_size", default=40, type=int, required=False, help='Time step size during inference')
parser.add_argument("--task_name", default='CHEBI', type=str, required=False)
parser.add_argument("--ckpt_path", default='./new_save/49.single_pretrain4/best(809999).th', type=str, required=False)
...
with torch.no_grad():
    for i, batch in enumerate(tqdm(test_loader)):
        ...
        dim_size = position_ids.shape[1]
        ''''''
        for i in range(position_ids.shape[0]):
            indices = torch.arange(batch['target_start'], dim_size - 2)
            permuted_indices = torch.randperm(indices.size(0))
            shuffled_indices = indices[permuted_indices]
            position_ids[i,batch['target_start']:dim_size-2] = position_ids[i,shuffled_indices]    
        ...
        generated_smiles = graph2smile_newpos(state,edge, position_ids, tokenizer)
        print(generated_smiles)
        output = model(input_ids=state, edge_input = edge, attention_mask=batch['attention_mask']
        ...
        print(generated_smiles)'''
        with open(f'./generation_results/{task_name}_{args.topk}_MBR_(49_80_1).txt', 'a+') as f_mbr:
