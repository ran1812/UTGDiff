...
def parse_args():
    ...
    parser.add_argument("--save_path", default='./new_save/49.single_pretrain4/', type=str, required=False)
        ...
        for batch in tqdm(train_loader):
            ...
            if dist.get_rank() == 0:
                if i % args.logging_steps == args.logging_steps - 1:
                    ...
                    logger.info(f'text_loss at step {i} is {text_loss / args.logging_steps}')
                    logger.info(f'accumulation_steps at step {i} is {args.accumulation_steps}')
                    lr = optimizer.param_groups[0]['lr']
                    logger.info(f'learning_rate at step {i} is {lr}')
                    logger.info('\n')
                    train_loss = .0
                    hybrid_loss_token = .0
                    base_loss_token = .0
            ...
            if i % args.eval_steps == args.eval_steps - 1:
                ...
                with torch.no_grad():
                    ...
                    if dist.get_rank() == 0:
                        torch.save({
                            'model': model.state_dict(),
                        }, f'./new_save/49.single_pretrain4/best({i}).th')
                model.train()
